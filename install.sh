...
main() {
    ...
    if [ "$SKIP_CHECKS" != "true" ]; then
        ...
        run_preflight_checks_parallel || exit 1
    else
    ...
    # Этап 2: Firewall (должен быть первым)
    setup_firewall || return 1
    
    # Этап 3: База данных (запускаем раньше, т.к. занимает много времени)
    setup_database &
    local db_pid=$!
    
    # ПАРАЛЛЕЛЬНАЯ УСТАНОВКА: Независимые компоненты
    log_info "🚀 Запуск независимых компонентов параллельно..."
    ...
    # Ожидание завершения базы данных
    log_info "Ожидание завершения установки базы данных..."
    wait $db_pid || return 1
    ok "База данных установлена"
    
    # Ожидание завершения независимых компонентов
    log_info "Ожидание завершения независимых компонентов..."
    
    wait $cache_pid || { log_error "Ошибка установки cache"; return 1; }
    ok "Cache установлен"
    
    wait $queue_pid || { log_error "Ошибка установки queue"; return 1; }
    ok "Queue установлен"
    
    wait $backend_pid || { log_error "Ошибка установки backend"; return 1; }
    ok "Backend установлен"
    
    wait $webserver_pid || { log_error "Ошибка установки webserver"; return 1; }
    ok "Webserver установлен"
    
    log_info "✅ Все компоненты установлены параллельно"
