...
1. **[РЕШЕНИЕ_ПРОБЛЕМЫ_BOOTSTRAP.md](РЕШЕНИЕ_ПРОБЛЕМЫ_BOOTSTRAP.md)** - ⭐ НАЧНИТЕ ЗДЕСЬ!
   - 3 метода установки (bootstrap, deploy, ручной)
   - Выбор подходящего метода
   - Решение проблем
   - **Обязательно прочитать перед установкой!**

2. **[УДАЛЕННАЯ_УСТАНОВКА.md](УДАЛЕННАЯ_УСТАНОВКА.md)** - 🌐 Для production
   - Bootstrap скрипт для удаленной установки
   - Требует HTTP сервер
...
4. **[deploy.sh](deploy.sh)** - ⭐ НОВОЕ - Для развертывания файлов!
   ```bash
   sudo ./deploy.sh /var/www/html/improved  # На веб-сервер
